...
const Icon = ({ d, size = 14, extra }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
    ...
    {extra}{d.map((p, i) => <path key={i} d={p} />)}
  </svg>
);

const IconPin = () => <Icon size={14}
  d={["M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"]}
  extra={<circle cx="12" cy="10" r="3" />} />;

const IconVirus = () => <Icon size={11}
...
const IconExternal = () => <Icon size={10}
  d={["M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6", "M10 14L21 3"]}
  extra={<polyline points="15 3 21 3 21 9" />} />;

const IconGlobe = () => <Icon size={10}
...
const ArticleCard = ({ art }) => {
  ...
  return (
    <a href={art.url || "#"} target="_blank" rel="noreferrer" className="rp-card">
      ...
      <div className="rp-card-disease">{art.disease_name || "Cập nhật chung"}</div>
      <div className="rp-card-meta">
        {art.location ? `Vietnam · ${art.location}` : "Vietnam"}
        {art.processed_at && ` · ${formatRelative(art.processed_at)}`}
      </div>
      <div className="rp-card-src"><IconExternal /> pipeline.web</div>
    </a>
  );
};
...
const RightPanel = ({ selectedLocation = null, onClearLocation }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        setAllArticles(await articleService.getAllArticles());
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  useEffect(() => { setActiveTab(0); }, [selectedLocation]);
  ...
  const counts = RISK_KEYS.map((r) => sourceArticles.filter((a) => a.risk_level === r).length);
  const total = sourceArticles.length;
  const listArticles = sourceArticles.filter((a) => a.risk_level === RISK_KEYS[activeTab]);
  const withLoc = listArticles.filter((a) => a.location);
  const withoutLoc = listArticles.filter((a) => !a.location);

  const diseaseList = Object.entries(
    sourceArticles.reduce((acc, a) => {
      ...
      if (a.risk_level) acc[a.disease_name][a.risk_level]++;
      return acc;
    }, {})
  )
    ...
    .slice(0, 6)
    .map(([name, data]) => {
      const dominantRisk = RISK_KEYS.reduce((best, r) =>
        (data[r] || 0) > (data[best] || 0) ? r : best, "LOW");
      return [name, data.total, dominantRisk];
    });

  const maxDis = diseaseList[0]?.[1] || 1;
  const maxVisible = isLocationMode ? 8 : 10;
  const overflow = listArticles.length - maxVisible;
  ...
  return (
    <div className="rp-root">
      ...
      {isLocationMode && selectedLocation.diseases?.length > 0 && (
        <div className="rp-loc-diseases">
          {selectedLocation.diseases.map((d, i) => (
            <span key={i} className="rp-dis-tag"><IconVirus /> {d}</span>
          ))}
        </div>
      )}
      ...
      <div className="rp-stats-row">
        ...
        <span className="rp-total-note">
          {total} tin{isLocationMode ? " tại đây" : " gần đây"}
        </span>
      </div>
      ...
      <div className="rp-section">
        ...
        <div className="rp-tabs">
          {TABS.map((lbl, i) => (
            <button key={i} className={`rp-tab${activeTab === i ? " active" : ""}`}
              onClick={() => setActiveTab(i)}>{lbl}</button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="rp-single-col">
            {listArticles.slice(0, 8).map((art) => <ArticleCard key={art.article_id} art={art} />)}
          </div>
        ) : (
          <div className="rp-two-col">
            <div className="rp-col">
              {withLoc.slice(0, 5).map((art) => <ArticleCard key={art.article_id} art={art} />)}
            </div>
            <div className="rp-col">
              {withoutLoc.slice(0, 5).map((art) => <ArticleCard key={art.article_id} art={art} />)}
            </div>
          </div>
        )}
      ...
      <div className="rp-section">
        ...
        <div className="rp-dis-hdr"><span>BỆNH</span><span>SỐ TIN</span></div>
        {loading ? (
